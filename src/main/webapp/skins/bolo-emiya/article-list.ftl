      ...
      <article class="article">
        <div class="article__info">
          <div class="article__info--left">
            <#if article.authorId?? && article.authorId !="">
              ...
              <#else>
                <a class="author" href="${servePath}/follow/articles/${article.articleAuthorId}" rel="nofollow">
                  <img class="author__avatar" src="${article.authorThumbnailURL}" alt="author avatar" />
                  <span class="author__name">
                    ${article.authorName}
                  </span>
            ...
            <#if article.articleCategory?? && article.articleCategory !="">
              <span>
                <a class="tags__name" href="${servePath}/category/${article.categoryURI}">
            ...
            <span class="tags">
              <#list article.articleTags?split(",") as articleTag>
                <a class="tags__name" rel="tag" href="${servePath}/tags/${articleTag?url('UTF-8')}">
                  ${articleTag}
                </a>
              </#list>
            </span>
        ...
        <div class="article__visit">
          <div class="article__visit--left">
            <span class="item--split createTime">
              ${article.articleCreateDate?string("yyyy-MM-dd")}
            </span>
            <#if interactive=="on"><a class="item item--split" href="${isRss?then(article.articlePermalink, servePath + article.articlePermalink)}#comments">
          ...
          <a class="article__visit--right" href="${isRss?then(article.articlePermalink, servePath + article.articlePermalink)}">阅读全文</a>
        </div>
      </article>
  </#list>
