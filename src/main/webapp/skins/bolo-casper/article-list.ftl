    ...
    <#list articles as article>
        <#assign isRss=(article.isRss!false)?c=="true">
            <article class="item<#if article_index % 6 ==0> item--large</#if>">
                <a href="${isRss?then(article.articlePermalink, servePath + article.articlePermalink)}" class="item__cover"
                    style="background-image: url(${article.articleImg1URL})">
                </a>
                <div class="item__main">
                    <#if article.articleCategory?? && article.articleCategory !="">
                        <span>
                            <a class="item__tag" href="${servePath}/category/${article.categoryURI}">
                    ...
                    <div class="fn__clear">
                        <#if article.authorId?? && article.authorId !="">
                            ...
                            <#else>
                                <a href="${servePath}/follow/articles/${article.articleAuthorId}"
                                    aria-label="${article.articleAuthorId}"
                                    class="vditor-tooltipped vditor-tooltipped__n item__avatar">
                                    <img src="${article.authorThumbnailURL}" />
                                </a>
                        </#if>
                        <#if article.articleCommentCount !=0>
                            <#if interactive=="on">
                                <a class="item__meta" href="${isRss?then(article.articlePermalink, servePath + article.articlePermalink)}#comments">
                                    ${article.articleCommentCount} ${commentLabel}
                                </a>
                                <#else>
                                    <a class="item__meta" href="${isRss?then(article.articlePermalink, servePath + article.articlePermalink)}">
    ...
    <nav class="pagination">
        ...
        <#list paginationPageNums as paginationPageNum>
            <#if paginationPageNum==paginationCurrentPageNum>
                ...
                <#else>
                    <a class="pagination__item" href="${servePath}${path}?p=${paginationPageNum}">
                        ${paginationPageNum}
                    </a>
            </#if>
        </#list>
        <#if paginationPageNums?last !=paginationPageCount>
            <span class="pagination__item pagination__item--omit">...</span>
            <a href="${servePath}${path}?p=${paginationPageCount}" class="pagination__item">
                ${paginationPageCount}
            </a>
