...
public class CategoryRepository extends AbstractRepository {
  ...
  public JSONObject getByTitle(final String categoryTitle) throws RepositoryException {
    final Query query =
        new Query()
            .setFilter(
                new PropertyFilter(Category.CATEGORY_TITLE, FilterOperator.EQUAL, categoryTitle))
            .setPageCount(1);
  ...
  public List<JSONObject> getMostUsedCategories(final int num) throws RepositoryException {
    ...
    final TagArticleRepository tagArticleRepository =
        beanManager.getReference(TagArticleRepository.class);
    final CategoryTagRepository categoryTagRepository =
        beanManager.getReference(CategoryTagRepository.class);
