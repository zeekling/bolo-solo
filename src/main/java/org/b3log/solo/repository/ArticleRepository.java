...
public class ArticleRepository extends AbstractRepository {
  ...
  /** Article cache. */
  @Inject private ArticleCache articleCache;
  ...
  @Override
  public void update(final String id, final JSONObject article, final String... propertyNames)
        ...
        new Query()
            .setFilter(
                CompositeFilterOperator.and(
                    new PropertyFilter(
                        Article.ARTICLE_RANDOM_DOUBLE, FilterOperator.GREATER_THAN_OR_EQUAL, mid),
                    new PropertyFilter(
                        Article.ARTICLE_RANDOM_DOUBLE, FilterOperator.LESS_THAN_OR_EQUAL, mid),
                    new PropertyFilter(
    ...
    if (0 != reminingSize) { // Query for remains
      ...
      query
          .setFilter(
              CompositeFilterOperator.and(
                  new PropertyFilter(
                      Article.ARTICLE_RANDOM_DOUBLE, FilterOperator.GREATER_THAN_OR_EQUAL, 0D),
                  new PropertyFilter(
                      Article.ARTICLE_RANDOM_DOUBLE, FilterOperator.LESS_THAN_OR_EQUAL, mid),
                  new PropertyFilter(
                      Article.ARTICLE_STATUS,
   ...
   * @param pageSize the specified page size(count of a page contains objects), MUST greater then
   *     {@code 0}
   * @return for example
   *     <pre>
   * {
   ...
   */
  public JSONObject getByAuthorId(
      final String authorId, final int currentPageNum, final int pageSize)
      throws RepositoryException {
    final Query query =
        new Query()
            .setFilter(
    ...
    final Query query =
        new Query()
            .setFilter(
                CompositeFilterOperator.and(
                    new PropertyFilter(
                        Article.ARTICLE_CREATED, FilterOperator.LESS_THAN, currentArticleCreated),
                    new PropertyFilter(
                        Article.ARTICLE_STATUS,
                        FilterOperator.EQUAL,
                        Article.ARTICLE_STATUS_C_PUBLISHED)))
