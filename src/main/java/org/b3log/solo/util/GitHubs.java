...
public final class GitHubs {
    ...
    try {
      ...
      while (true) {
        HttpResponse res =
            HttpRequest.get(
                    "https://api.github.com/users/"
                        + githubUserId
                        + "/repos?sort=updated&per_page=50&type=public&page="
                        + page)
        ...
        for (int i = 0; i < result.length(); i++) {
          ...
          compatibleObject.put(
              "githubrepoOwnerId", resultObject.optJSONObject("owner").optString("id"));
          compatibleObject.put(
              "githubrepoStargazersCount", resultObject.optLong("stargazers_count"));
          compatibleObject.put("githubrepoWatchersCount", resultObject.optLong("watchers_count"));
          compatibleObject.put(
              "githubrepoOwnerLogin", resultObject.optJSONObject("owner").optString("login"));
      ...
      LOGGER.log(
          Level.ERROR,
          "Gets GitHub repos failed because the request has been reached GitHub's limit, try again at later.");

      return null;
    } catch (final Exception e) {
      LOGGER.log(
          Level.ERROR,
          "Gets GitHub repos failed, please check your network connection to github.com");
  ...
  /**
   * Updates a file by the specified personal access token, GitHub login name, repo name, file path
   * and file content.
   *
  ...
  public static boolean updateFile(
    ...
    try {
      HttpResponse response =
          HttpRequest.get("https://api.github.com/repos/" + fullRepoName + "/git/trees/master")
      ...
      response =
          HttpRequest.put("https://api.github.com/repos/" + fullRepoName + "/contents/" + filePath)
              .header("Authorization", "token " + pat)
              .connectionTimeout(7000)
              .timeout(60000 * 2)
              .header("User-Agent", Solos.USER_AGENT)
      ...
      if (200 != statusCode && 201 != statusCode) {
        LOGGER.log(
            Level.ERROR,
            "Updates repo [" + repoName + "] file [" + filePath + "] failed: " + responseBody);
        return false;
      }
      return true;
    } catch (final Exception e) {
      LOGGER.log(
          Level.ERROR,
          "Updates repo [" + repoName + "] file [" + filePath + "] failed: " + e.getMessage());
      return false;
    }
  }

  /**
   * Creates or updates a GitHub repository by the specified personal access token, GitHub login
   * name, repo name repo desc and repo homepage.
   *
   * @param pat the specified personal access token
   * @param loginName the specified GitHub login name
   * @param repoName the specified repo name
   ...
   * @return {@code true} if ok, returns {@code false} if failed
   */
  public static boolean createOrUpdateGitHubRepo(
      final String pat,
      final String loginName,
      final String repoName,
      final String repoDesc,
      final String repoHomepage) {
    try {
      final JSONObject body =
          new JSONObject()
              .put("name", repoName)
      ...
      HttpResponse response =
          HttpRequest.post("https://api.github.com/user/repos")
              .header("Authorization", "token " + pat)
              .connectionTimeout(7000)
              .timeout(30000)
              .header("User-Agent", Solos.USER_AGENT)
      ...
      response =
          HttpRequest.patch("https://api.github.com/repos/" + loginName + "/" + repoName)
              .header("Authorization", "token " + pat)
              .connectionTimeout(7000)
              .timeout(30000)
              .header("User-Agent", Solos.USER_AGENT)
