...
public final class Solos {
  ...
  public static final String USER_AGENT =
      "Solo/" + SoloServletListener.VERSION + "; +https://github.com/b3log/solo";

  /** Bolo User-Agent. */
  public static final String BOLO_USER_AGENT =
      "Bolo/" + SoloServletListener.BOLO_VERSION_SOURCE + "; +https://github.com/adlered/bolo-solo";
  ...
  static {
    ...
    if (!Markdowns.LUTE_AVAILABLE) {
      try {
        ...
        if (Markdowns.LUTE_AVAILABLE) {
          LOGGER.log(
              Level.INFO, "lute_http configure detected [url=" + Markdowns.LUTE_ENGINE_URL + "]");
        }
  ...
  public static void enableWelfareLuteService() {
    ...
    if (status) {
      ...
      needTest = true;
      testErrorMsg =
          "The connection to the public welfare Lute service failed. Lute rendering will be disabled and the built-in renderer will be used.";
    } else if (Markdowns.LUTE_AVAILABLE && !LUTE_URL.equals(Markdowns.LUTE_ENGINE_URL)) {
      needTest = true;
      testErrorMsg =
          "The connection to the private welfare Lute service failed. Lute rendering will be disabled and the built-in renderer will be used.";
    } else {
      Markdowns.LUTE_AVAILABLE = status;
  ...
  public static JSONObject newFail() {
    return new JSONObject()
        .put(Keys.CODE, -1)
        .put(Keys.MSG, "System is abnormal, please try again later");
  }
  ...
  public static JSONObject getUploadToken(final RequestContext context) {
    try {
      ...
      final JSONObject requestJSON =
          new JSONObject().put(User.USER_NAME, userName).put(UserExt.USER_B3_KEY, userB3Key);
      final HttpResponse res =
          HttpRequest.post("https://" + Global.HACPAI_DOMAIN + "/apis/upload/token")
      ...
      LOGGER.log(
          Level.ERROR, "Gets Hacpai upload token failed, Wrong Hacpai Username / B3log key.");
   ...
   *
   * <p>使用“noindex”阻止搜索引擎将您的网页编入索引 https://support.google.com/webmasters/answer/93710?hl=zh-Hans
   *
   * @param context the specified context
   */
  ...
  /**
   * Checks whether the current request is made by a logged in user (including default user and
   * administrator lists in <i>users</i>).
   *
   * @param context the specified request context
   * @return {@code true} if the current request is made by logged in user, returns {@code false}
   *     otherwise
   */
   ...
   *
   * @param context the specified request context
   * @return {@code true} if the current request is made by logged in administrator, returns {@code
   *     false} otherwise
   */
   ...
   *
   * @param context the specified request context
   * @return {@code true} if the current request is made by logged in administrator, author, returns
   ...
   *
   * <p>Checks session, if not represents, checks article property {@link Article#ARTICLE_VIEW_PWD
   * view password}.
   *
  ...
  public static boolean needViewPwd(final RequestContext context, final JSONObject article) {
    ...
    return !(null != currentUser
        && !Role.VISITOR_ROLE.equals(currentUser.optString(User.USER_ROLE)));
  }
   ...
   *
   * @param path the specified path
   * @return page number, returns {@value #DEFAULT_PAGE_SIZE} if the specified request URI can not
   ...
   *
   * @param path the specified path
   * @return page number, returns {@value #DEFAULT_WINDOW_SIZE} if the specified request URI can not
   *     convert to an number
   */
