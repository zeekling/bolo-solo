...
public class PermalinkQueryService {
  ...
  private static final Pattern pattern =
      Pattern.compile("/articles/(?:\\d{4}/\\d{2}/\\d{2}/)?\\d+\\.html");

  /**
   * Checks whether the specified article permalink matches the system generated format pattern
   * ("/articles/yyyy/MM/dd/${articleId}.html"). format pattern ("/articles/${articleId}.html").
   *
  ...
  /**
   * Checks whether the specified permalink is a {@link
   * #invalidArticlePermalinkFormat(java.lang.String) invalid article permalink format} and {@link
   * #invalidPagePermalinkFormat(java.lang.String) invalid page permalink format}.
   *
   * @param permalink the specified permalink
   ...
   *
   * <p>A URI starts with one of {@link PermalinkQueryService#RESERVED_LINKS reserved links} will be
   * treated as reserved link.
   *
  ...
  public boolean exist(final String permalink) {
    try {
      ...
      LOGGER.log(
          Level.ERROR,
          "Determines whether the permalink[" + permalink + "] exists failed, returns true",
          e);
