...
public class InitService {
  ...
  public boolean isInited() {
    ...
    try {
      ...
      if (!inited && !printedInitMsg) {
        LOGGER.log(
            Level.WARN, "Bolo has not been initialized, please open your browser to init Bolo");
        printedInitMsg = true;
      }
  ...
  public void initSpecificTables(final List<String> modelNames) {
    ...
    final List<String> tableNames =
        modelNames.stream().map(modelName -> tablePrefix + modelName).collect(Collectors.toList());
    ...
    try {
      ...
      for (final CreateTableResult createTableResult : ret) {
        LOGGER.log(
            Level.INFO,
            "Creates table result [tableName={0}, isSuccess={1}]",
            createTableResult.getName(),
  ...
  public void initTables() {
    ...
    LOGGER.info(
        "It's your first time setup Bolo, initialize tables in database ["
   ...
   *     "userAvatar": "", // optional "userB3Key": "", // optional "userGitHubId": "" // optional }
   */
   ...
   *
   * @param article the specified article, for example, { ...., "oId": "", "articleCreateDate":
   *     java.util.Date, .... }
   * @throws RepositoryException repository exception
   */
