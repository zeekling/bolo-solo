...
public class ArticleMgmtService {
  ...
  @Inject private ArticleQueryService articleQueryService;
  ...
  @Inject private CategoryMgmtService categoryMgmtService;
   ...
   *
   * @param articleId the given article id
   * @param top the specified flag, {@code true} to top, {@code false} to cancel top
   * @throws ServiceException service exception
   */
   ...
   *
   * @param requestJSONObject the specified request json object, for example, { "article": { "oId":
   *     "", "articleTitle": "", "articleAbstract": "", "articleContent": "", "articleTags":
   *     "tag1,tag2,tag3", // optional, default set "待分类" "categories": "cat1,cat2,cat3",
   *     "articlePermalink": "", // optional "articleStatus": int, // 0: published, 1: draft
   *     "articleSignId": "", // optional "articleCommentable": boolean, "articleViewPwd": "" } }
   * @throws ServiceException service exception
   */
  public void updateArticle(final JSONObject requestJSONObject) throws ServiceException {
    ...
    try {
      ...
      final String permalink =
          getPermalinkForUpdateArticle(oldArticle, article, oldArticle.optLong(ARTICLE_CREATED));
      ...
      final boolean publishNewArticle =
          Article.ARTICLE_STATUS_C_DRAFT == oldArticle.optInt(ARTICLE_STATUS)
              && Article.ARTICLE_STATUS_C_PUBLISHED == article.optInt(ARTICLE_STATUS);
   ...
   *
   * @param requestJSONObject the specified request json object, for example, { "article": {
   *     "articleAuthorId": "", "articleTitle": "", "articleAbstract": "", "articleContent": "",
   *     "articleTags": "tag1,tag2,tag3", "articleStatus": int, // 0: published, 1: draft
   *     "articlePermalink": "", // optional "postToCommunity": boolean, // optional
   *     "articleSignId": "" // optional, default is "0", "articleCommentable": boolean,
   *     "articleCommentCount": int, // optional, default is 0 "articleViewPwd": "", "oId": "" //
   *     optional, generate it if not exists this key } }
  ...
  /**
   * Updates the random values of articles fetched with the specified update count.
   *
   ...
   *
   * <ul>
   *   <li>Un-tags old article, decrements tag reference count
   *   <li>Removes old article-tag relations
   *   <li>Saves new article-tag relations with tag reference count
   * </ul>
   *
  ...
  private void processTagsForArticleUpdate(final JSONObject oldArticle, final JSONObject newArticle)
    ...
    removeTagArticleRelations(
        oldArticleId, 0 == tagIdsDropped.length ? new String[] {"l0y0l"} : tagIdsDropped);
  ...
  /**
   * Removes tag-article relations by the specified article id and tag ids of the relations to be
   * removed.
   *
   * <p>Removes all relations if not specified the tag ids.
   *
   * @param articleId the specified article id
   ...
   * @throws RepositoryException repository exception
   */
  private void addTagArticleRelation(final JSONArray tags, final JSONObject article)
    ...
    for (int i = 0; i < tags.length(); i++) {
      ...
      tagArticleRelation.put(
          Article.ARTICLE + "_" + Keys.OBJECT_ID, article.optString(Keys.OBJECT_ID));
   ...
   *
   * @param article the specified article, for example, { "oId": "", .... }
   * @throws RepositoryException repository exception
   */
  private void archiveDate(final JSONObject article) throws RepositoryException {
    ...
    if (null == archiveDateArticleRepository.getByArticleId(articleId)) {
      ...
      archiveDateArticleRelation.put(
          ArchiveDate.ARCHIVE_DATE + "_" + Keys.OBJECT_ID, archiveDate.optString(Keys.OBJECT_ID));
   ...
   *
   * <p>Some properties of an article are not been changed while article updating, these properties
   * are called 'auto' properties.
   *
   * <p>The property(named {@value org.b3log.solo.model.Article#ARTICLE_RANDOM_DOUBLE}) of the
   ...
   * @throws JSONException json exception
   */
  private void fillAutoProperties(final JSONObject oldArticle, final JSONObject article)
    ...
    try {
      ...
      LOGGER.log(
          Level.ERROR, "Customize article create time: timestamp syntax error, using earlier...");
  ...
  /**
   * Gets article permalink for updating article with the specified old article, article, created
   * at.
   *
   * @param oldArticle the specified old article
   * @param article the specified article
   ...
   * @throws JSONException json exception
   */
  private String getPermalinkForUpdateArticle(
      final JSONObject oldArticle, final JSONObject article, final long created)
    ...
    if (!oldPermalink.equals(ret)) {
      if (StringUtils.isBlank(ret)) {
        ret =
            "/articles/"
                + DateFormatUtils.format(created, "yyyy/MM/dd")
                + "/"
                + articleId
                + ".html";
      }
   ...
   * @throws JSONException json exception
   */
  private static boolean tagExists(final String tagTitle, final List<JSONObject> tags)
      throws JSONException {
