...
public class StatisticMgmtService {
  ...
  /** Statistic cache. */
  @Inject private StatisticCache statisticCache;
   ...
   *
   * <p>A "served request" is a request a URI as former one. For example, if a client is request
   * "/test", all requests from the client subsequent in 24 hours will be treated as served
   * requests, requested URIs save in client cookie (name: "visited").
   *
   * <p>If the specified request has not been served, appends the request URI in client cookie.
   *
   * <p>Sees this issue (https://github.com/b3log/solo/issues/44) for more details.
   *
   * @param context the specified request context
   * @param response the specified response
   * @return {@code true} if the specified request has been served, returns {@code false} otherwise
   */
  public static boolean hasBeenServed(
      final RequestContext context, final HttpServletResponse response) {
    ...
    try {
      ...
      if (needToCreate) {
        final StringBuilder builder =
            new StringBuilder("[").append("\"").append(request.getRequestURI()).append("\"]");
   ...
   *
   * <p>If it is a search engine bot made the specified request, will NOT increment blog statistic
   * view count.
   *
   * <p>There is a cron job (/console/stat/viewcnt) to flush the blog view count from cache to
   * datastore.
   *
   * @param context the specified request context
   * @param response the specified response
   ...
   */
  public void incBlogViewCount(final RequestContext context, final HttpServletResponse response)
   ...
   */
  private void updateStatistic(final String id, final JSONObject statistic)
