...
public class ArticleQueryService {
  ...
  @Inject private CategoryQueryService categoryQueryService;
   ...
   * @return result
   */
  public JSONObject searchKeyword(
      final String keyword, final int currentPageNum, final int pageSize) {
    ...
    try {
              ...
              .setFilter(
                  CompositeFilterOperator.and(
                      ...
                      CompositeFilterOperator.or(
                          new PropertyFilter(
                              Article.ARTICLE_TITLE, FilterOperator.LIKE, "%" + keyword + "%"),
                          new PropertyFilter(
                              Article.ARTICLE_TAGS_REF, FilterOperator.LIKE, "%" + keyword + "%"),
                          new PropertyFilter(
                              Article.ARTICLE_CONTENT, FilterOperator.LIKE, "%" + keyword + "%"))))
      ...
      final List<JSONObject> articles =
          CollectionUtils.jsonArrayToList(result.optJSONArray(Keys.RESULTS));
   ...
   */
  public JSONObject getCategoryArticles(
      final String categoryId, final int currentPageNum, final int pageSize)
    ...
    try {
      ...
      final JSONArray categoryTags =
          categoryTagRepository
              .getByCategoryId(categoryId, 1, Integer.MAX_VALUE)
      ...
      final int tagArticlesCount =
          tagArticlesCountResult == null ? 0 : tagArticlesCountResult.get(0).optInt("C");
   ...
   * @throws ServiceException service exception
   */
  public boolean canAccessArticle(final String articleId, final JSONObject user)
      throws ServiceException {
   ...
   *
   * <p>The specified article has a property {@value Article#ARTICLE_AUTHOR_ID}, this method will
   * use this property to get a user from users.
   *
   * <p>If can't find the specified article's author (i.e. the author has been removed by
  ...
  public JSONObject getAuthor(final JSONObject article) throws ServiceException {
    try {
      ...
      LOGGER.log(
          Level.ERROR, "Gets author of article [id={0}] failed", article.optString(Keys.OBJECT_ID));
   ...
   *
   * <p><b>Note</b>: The article content and abstract is raw (no editor type processing).
   *
   ...
   *
   * <p>Specified the "excludes" for results properties exclusion.
   *
   * @param requestJSONObject the specified request json object, for example,
   *     "paginationCurrentPageNum": 1, "paginationPageSize": 20, "paginationWindowSize": 10,
   *     "articleStatus": int, "keyword": "", // Optional search keyword "excludes": ["", ....], //
   *     Optional "enableArticleUpdateHint": bool // Optional see {@link Pagination} for more
   *     details
   * @return for example,
   *     <pre>
   * {
  ...
  public JSONObject getArticles(final JSONObject requestJSONObject) {
    ...
    try {
      ...
      final int articleStatus =
          requestJSONObject.optInt(Article.ARTICLE_STATUS, Article.ARTICLE_STATUS_C_PUBLISHED);
      ...
      for (int i = 0; i < articles.length(); i++) {
        ...
        try {
          ...
          JSONObject cate =
              categoryTagRepository.getByTagId(article.optString("oId"), 1, Integer.MAX_VALUE);
   ...
   * @throws ServiceException service exception
   */
  public JSONObject getArticlesByTag(
      final String tagId, final int currentPageNum, final int pageSize) throws ServiceException {
    try {
      ...
      for (int i = 0; i < tagArticleRelations.length(); i++) {
        ...
        final String articleId =
            tagArticleRelation.getString(Article.ARTICLE + "_" + Keys.OBJECT_ID);
      ...
      for (final JSONObject article : articles) {
        article.put(Article.ARTICLE_CREATE_TIME, article.getLong(Article.ARTICLE_CREATED));
        article.put(
            Article.ARTICLE_T_CREATE_DATE, new Date(article.getLong(Article.ARTICLE_CREATED)));
        article.put(
            Article.ARTICLE_T_UPDATE_DATE, new Date(article.optLong(Article.ARTICLE_UPDATED)));
  ...
  /**
   * Gets a list of published articles with the specified archive date id, current page number and
   * page size.
   *
  ...
  public List<JSONObject> getArticlesByArchiveDate(
      final String archiveDateId, final int currentPageNum, final int pageSize)
      throws ServiceException {
    try {
      ...
      final String query =
          "SELECT\n"
              ...
              + " ?";
      final List<JSONObject> articles =
          articleRepository.select(query, archiveDateId, (currentPageNum - 1) * pageSize, pageSize);
      for (final JSONObject article : articles) {
        article.put(Article.ARTICLE_CREATE_TIME, article.getLong(Article.ARTICLE_CREATED));
        article.put(
            Article.ARTICLE_T_CREATE_DATE, new Date(article.getLong(Article.ARTICLE_CREATED)));
    ...
    try {
      ...
      for (int i = 0; i < maxTagCnt; i++) { // XXX: should average by tag?
        ...
        final int relationSize =
            displayCnt < tagArticleRelations.length() ? displayCnt : tagArticleRelations.length();
      ...
      final List<Integer> randomIntegers =
          CollectionUtils.getRandomIntegers(0, articles.size() - 1, displayCnt);
      final List<JSONObject> ret = new ArrayList<>();
   ...
   * @throws ServiceException service exception
   */
  public JSONObject getArticlesByAuthorId(
      final String authorId, final int currentPageNum, final int pageSize) throws ServiceException {
    try {
      ...
      LOGGER.log(
          Level.ERROR,
          "Gets articles by author id failed [authorId="
              + authorId
              + ", currentPageNum="
              + currentPageNum
              + ", pageSize="
   ...
   *
   * <p>Invoking this method dose not effect on article view count.
   *
   ...
   * @throws ServiceException service exception
   */
  public String getArticleContent(final RequestContext context, final String articleId)
      throws ServiceException {
    if (StringUtils.isBlank(articleId)) {
      return null;
    }
  ...
  /**
   * Converts the content and abstract for each of the specified articles to HTML if that is saved
   * by Markdown editor.
   *
  ...
  /**
   * Converts the content and abstract for the specified article to HTML if it is saved by Markdown
   * editor.
   *
   * @param article the specified article
   */
   ...
   *
   * <p>Remains the following properties:
   *
   * <ul>
   *   <li>{@link Article#ARTICLE_TITLE article title}
   *   <li>{@link Article#ARTICLE_PERMALINK article permalink}
   * </ul>
   *
   * <p>The batch version of method {@link #removeUnusedProperties(org.json.JSONObject)}.
   *
   * @param articles the specified articles
