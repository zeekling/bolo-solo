...
public class CategoryQueryService {
  ...
  public List<JSONObject> getTags(final String categoryId) {
    ...
    final Query query =
        new Query()
            .setFilter(
                new PropertyFilter(
                    Category.CATEGORY + "_" + Keys.OBJECT_ID, FilterOperator.EQUAL, categoryId));
    try {
   ...
   *
   * @param requestJSONObject the specified request json object, for example, "categoryTitle": "",
   *     // optional "paginationCurrentPageNum": 1, "paginationPageSize": 20,
   *     "paginationWindowSize": 10 see {@link Pagination} for more details
   * @return for example,
   *     <pre>
   * {
   *     "pagination": {
  ...
  /**
   * Whether a tag specified by the given tag title in a category specified by the given category
   * id.
   *
  ...
  public boolean containTag(final String tagTitle, final String categoryId) {
    try {
          ...
          new Query()
              .setFilter(
                  CompositeFilterOperator.and(
                      new PropertyFilter(
                          Category.CATEGORY + "_" + Keys.OBJECT_ID,
                          FilterOperator.EQUAL,
                          categoryId),
                      new PropertyFilter(
      ...
      LOGGER.log(
          Level.ERROR,
          "Check category tag [tagTitle=" + tagTitle + ", categoryId=" + categoryId + "] failed",
          e);
