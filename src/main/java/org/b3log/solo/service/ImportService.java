...
public class ImportService {
  ...
  /**
   * Imports markdowns files as articles. See <a href="https://ld246.com/article/1498490209748">Solo
   * 支持 Hexo/Jekyll 数据导入</a> for more details.
   */
  public void importMarkdowns() {
    new Thread(
            () -> {
              ...
              final Collection<File> mds =
                  FileUtils.listFiles(new File(markdownsPath), new String[] {"md"}, true);
              if (mds.isEmpty()) {
                return;
              }
              ...
              for (final File md : mds) {
                ...
                try {
                  ...
                  LOGGER.info(
                      "Imported article [" + article.optString(Article.ARTICLE_TITLE) + "]");
                  succCnt++;
