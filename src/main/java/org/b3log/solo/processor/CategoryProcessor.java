...
public class CategoryProcessor {
  ...
  public void getCategoryArticlesByPage(final RequestContext context) {
    ...
    try {
      ...
      final JSONObject articlesResult =
          articleQueryService.getCategoryArticles(categoryId, currentPageNum, pageSize);
  ...
  public void showCategoryArticles(final RequestContext context) {
    ...
    try {
      ...
      final int currentPageNum = Paginator.getPage(request);
      LOGGER.log(
          Level.DEBUG, "Category [URI={0}, currentPageNum={1}]", categoryURI, currentPageNum);
      // 读取category表，无关键操作
      final JSONObject category = categoryQueryService.getByURI(categoryURI);
      if (null == category) {
        context.sendError(HttpServletResponse.SC_NOT_FOUND);
      ...
      final List<Integer> pageNums =
          (List) result.optJSONObject(Pagination.PAGINATION).opt(Pagination.PAGINATION_PAGE_NUMS);
