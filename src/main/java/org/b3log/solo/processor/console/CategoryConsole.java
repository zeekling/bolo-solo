...
public class CategoryConsole {
  ...
  public void getCategory(final RequestContext context) {
    ...
    try {
      ...
      for (final JSONObject tag : tags) {
        if (null == tag
            || !tag.has(Tag.TAG_TITLE)) { // 修复修改分类时空指针错误 https://github.com/b3log/solo/pull/12876
          continue;
        }
   ...
   * /console/categories/1/10/20, means the current page is 1, the page size is 10, and the window
   * size is 20.
   *
   * <p>Renders the response with a json object, for example,
   *
  ...
  public void getCategories(final RequestContext context) {
    ...
    try {
      ...
      final String path =
          requestURI.substring((Latkes.getContextPath() + "/console/categories/").length());
