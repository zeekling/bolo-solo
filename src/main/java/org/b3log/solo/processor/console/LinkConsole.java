...
public class LinkConsole {
  ...
  /** Link query service. */
  @Inject private LinkQueryService linkQueryService;

  /** Link management service. */
  @Inject private LinkMgmtService linkMgmtService;
   ...
   * /console/links/1/10/20, means the current page is 1, the page size is 10, and the window size
   * is 20.
   *
   * <p>Renders the response with a json object, for example,
   *
  ...
  public void getLinks(final RequestContext context) {
    ...
    try {
      ...
      final String path =
          requestURI.substring((Latkes.getContextPath() + "/console/links/").length());
