...
public class ArticleConsole {
  ...
  @Inject CategoryTagRepository categoryTagRepository;
  ...
  public void getArticleThumbs(final RequestContext context) {
    ...
    result.put(
        "data",
        urls.stream()
            .map(url -> Images.imageSize(url, width, height))
   ...
   * /console/articles/status/published/1/10/20, means the current page is 1, the page size is 10,
   * and the window size is 20.
   *
   * <p>Renders the response with a json object, for example,
   *
   * <pre>
    ...
    try {
      ...
      requestJSONObject.put(
          ...
          "published".equals(status)
              ...
              : Article.ARTICLE_STATUS_C_DRAFT);
   ...
   *
   * <p>The specified request json object, for example,
   *
   * <pre>
   * {
   *     "article": {
  ...
  public void addArticle(final RequestContext context) {
    ...
    try {
      ...
      requestJSONObject
          ...
          .put(Article.ARTICLE_AUTHOR_ID, currentUser.getString(Keys.OBJECT_ID));
