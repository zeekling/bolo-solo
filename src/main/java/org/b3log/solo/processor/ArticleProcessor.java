...
public class ArticleProcessor {
  ...
  @Inject private ArchiveDateQueryService archiveDateQueryService;

  /** User query service. */
  @Inject private UserQueryService userQueryService;

  /** Article management service. */
  @Inject private ArticleMgmtService articleMgmtService;

  @Inject private FollowArticleCache followArticleCache;
  ...
  public void onArticlePwdForm(final RequestContext context) {
    try {
      ...
      if (article.getString(Article.ARTICLE_VIEW_PWD).equals(pwdTyped)) {
        ...
        if (null != session) {
          Map<String, String> viewPwds =
              (Map<String, String>) session.getAttribute(Common.ARTICLES_VIEW_PWD);
          if (null == viewPwds) {
  ...
  public void getRelevantArticles(final RequestContext context) {
    ...
    final List<JSONObject> relevantArticles =
        articleQueryService.getRelevantArticles(article, preference);
    ...
    try {
      ...
      final List<JSONObject> articles =
          org.b3log.latke.util.CollectionUtils.jsonArrayToList(
              result.getJSONArray(Article.ARTICLES));
  ...
  public void getTagArticlesByPage(final RequestContext context) {
    ...
    final int currentPageNum = Paginator.getPage(request);
    Stopwatchs.start(
        "Get Tag-Articles Paged [tagTitle=" + tagTitle + ", pageNum=" + currentPageNum + ']');
    try {
      jsonObject.put(Keys.STATUS_CODE, true);
  ...
  public void getArchivesArticlesByPage(final RequestContext context) {
    ...
    Stopwatchs.start(
        "Get Archive-Articles Paged [archive="
            + archiveDateString
            + ", pageNum="
            + currentPageNum
            + ']');
    try {
      ...
      final JSONObject archiveQueryResult =
          archiveDateQueryService.getByArchiveDateString(archiveDateString);
      ...
      final int articleCount =
          archiveDateQueryService.getArchiveDatePublishedArticleCount(archiveDateId);
      final int pageCount = (int) Math.ceil((double) articleCount / (double) pageSize);

      final List<JSONObject> articles =
          articleQueryService.getArticlesByArchiveDate(archiveDateId, currentPageNum, pageSize);
      dataModelService.setArticlesExProperties(context, articles, preference);
  ...
  public void getAuthorsArticlesByPage(final RequestContext context) {
    ...
    Stopwatchs.start(
        "Get Author-Articles Paged [authorId=" + authorId + ", pageNum=" + currentPageNum + ']');
    try {
      jsonObject.put(Keys.STATUS_CODE, true);
      ...
      final JSONObject articlesResult =
          articleQueryService.getArticlesByAuthorId(authorId, currentPageNum, pageSize);
      final List<JSONObject> articles =
  ...
  public void showFollowUserArticles(final RequestContext context) {
    ...
    try {
      ...
      Skins.fillLangs(
          ...
          dataModel);
      final List<JSONObject> articles =
          followArticleCache.getFollowArticles(followName).values().stream()
              .sorted(
  ...
  public void showArticle(final RequestContext context) {
    ...
    try {
      ...
      final String metaDescription =
          Jsoup.parse(article.optString(Article.ARTICLE_ABSTRACT)).text();
  ...
  private void prepareShowAuthorArticles(
    ...
    dataModel.put(
        Pagination.PAGINATION_PREVIOUS_PAGE_NUM,
  ...
  private String prepareShowArchiveArticles(
    ...
    if ("en".equals(language)) {
      ...
      ret =
          year + " " + dataModel.get("yearLabel") + " " + month + " " + dataModel.get("monthLabel");
    }
   ...
   */
  private void prepareShowArticle(
      final JSONObject preference, final Map<String, Object> dataModel, final JSONObject article)
      throws Exception {
    article.put(
        Common.COMMENTABLE,
        preference.getBoolean(Option.ID_C_COMMENTABLE)
            && article.getBoolean(Article.ARTICLE_COMMENTABLE));
    ...
    if (null != nextArticle) {
      dataModel.put(
          Common.NEXT_ARTICLE_PERMALINK, nextArticle.getString(Article.ARTICLE_PERMALINK));
    ...
    if (null != previousArticle) {
      dataModel.put(
          Common.PREVIOUS_ARTICLE_PERMALINK, previousArticle.getString(Article.ARTICLE_PERMALINK));
      dataModel.put(
          Common.PREVIOUS_ARTICLE_TITLE, previousArticle.getString(Article.ARTICLE_TITLE));
      dataModel.put(
          Common.PREVIOUS_ARTICLE_ABSTRACT, previousArticle.getString(Article.ARTICLE_ABSTRACT));
