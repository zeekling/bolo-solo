...
public class KanBanNiangProcessor {
  ...
  public void randomModel(final RequestContext context) {
    ...
    try {
      ...
      try {
        model =
            optionRepository.get(Option.ID_C_KANBANNIANG_SELECTOR).optString(Option.OPTION_VALUE);
        if (model.isEmpty()) {
          throw new NullPointerException();
        }
      } catch (NullPointerException e) {
        try (final InputStream inputStream =
            servletContext.getResourceAsStream(assets + "/model-list.json")) {
      ...
      try (final InputStream modelResource =
          servletContext.getResourceAsStream(assets + "/model/" + model + "/index.json")) {
        ...
        if (textures.length() == 0) {
          try (final InputStream texturesRes =
              servletContext.getResourceAsStream(assets + "/model/" + model + "/textures.json")) {
   ...
   */
  @RequestProcessing(
      value = "/plugins/kanbanniang/assets/absoluteRandomModel",
