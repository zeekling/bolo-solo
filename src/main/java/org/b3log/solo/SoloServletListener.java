...
public final class SoloServletListener extends AbstractServletListener {
  ...
  public void contextInitialized(final ServletContextEvent servletContextEvent) {
    ...
    Stopwatchs.end();
    LOGGER.log(
        Level.DEBUG, "Stopwatch: {0}{1}", Strings.LINE_SEPARATOR, Stopwatchs.getTimingStat());
  ...
  public void requestInitialized(final ServletRequestEvent servletRequestEvent) {
    ...
    if (!Solos.isBot(httpServletRequest)) {
      final StatisticMgmtService statisticMgmtService =
          beanManager.getReference(StatisticMgmtService.class);
   ...
   * Loads skin.
   *
   * <p>Loads skin from repository, loads skins from skin directory then sets it into preference if
   * the skins changed.
   */
  private void loadPreference() {
    ...
    try {
      ...
      final String showClodeBlockLn =
          preference.optString(org.b3log.solo.model.Option.ID_C_SHOW_CODE_BLOCK_LN);
  ...
  private void registerEventHandlers() {
    ...
    try {
      ...
      final FishPiArticleSender fishPiArticleSender =
          beanManager.getReference(FishPiArticleSender.class);
      eventManager.registerListener(fishPiArticleSender);
      final FishPiArticleUpdater fishPiArticleUpdater =
          beanManager.getReference(FishPiArticleUpdater.class);
      eventManager.registerListener(fishPiArticleUpdater);
      final DeleteArticleListener deleteArticleListener =
          beanManager.getReference(DeleteArticleListener.class);
      eventManager.registerListener(deleteArticleListener);
      final FollowArticleRefresher followArticleRefresher =
          beanManager.getReference(FollowArticleRefresher.class);
      eventManager.registerListener(followArticleRefresher);
      final DeleteFollowListener deleteFollowListener =
          beanManager.getReference(DeleteFollowListener.class);
  ...
  /**
   * Resolve skin (template) for the specified HTTP servlet request. 前台皮肤切换
   ...
   *
   * <p>改进皮肤加载校验 https://github.com/b3log/solo/issues/12548
   */
  private static void validateSkin() {
    ...
    if (StringUtils.isBlank(skinName)) {
      LOGGER.log(
          Level.ERROR,
          "Can't load the default skins, please make sure skin ["
              + skinDirName
  ...
  public static void routeConsoleProcessors() {
    ...
    DispatcherServlet.get(
        "/console/categories/{page}/{pageSize}/{windowSize}", categoryConsole::getCategories);
    ...
    DispatcherServlet.get(
        "/console/comments/{page}/{pageSize}/{windowSize}", commentConsole::getComments);
    ...
    DispatcherServlet.get(
        "/console/follows/{page}/{pageSize}/{windowSize}", followConsole::getFollows);
    ...
    DispatcherServlet.get(
        "/console/plugins/{page}/{pageSize}/{windowSize}", pluginConsole::getPlugins);
