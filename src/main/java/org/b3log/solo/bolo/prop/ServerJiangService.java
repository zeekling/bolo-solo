...
public class ServerJiangService {
  ...
  public static boolean send(String title, String desp, String sendKey) {
    final HttpResponse response =
        HttpRequest.post("https://sctapi.ftqq.com/" + sendKey + ".send")
            .form("title", title)
            .form("desp", desp)
  ...
  public static void remindAdmin(String blogSite, String user, String comment, String blogTitle) {
    ...
    if (!sendKey.isEmpty()) {
      ...
      if (!send(title, desp, sendKey)) {
        LOGGER.log(
            Level.ERROR,
            "Server Jiang Remind send failed, please check your \"sendKey\" option. Free accounts can only send 5 times a day, maybe the limit is reached?");
      } else {
        LOGGER.log(
            Level.INFO, "Server Jiang Remind sent. [title=" + title + ", desp=" + desp + "]");
      }
    }
  }
}
