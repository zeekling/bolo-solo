...
public class BackupService {
  ...
  public void syncGitHubRepos(final RequestContext context) {
    ...
    try {
      ...
      context
          .renderJSON()
          .renderMsg("Sync github repos to article successfully. [githubId=" + githubId + "]");
  ...
  @RequestProcessing(
      value = "/prop/backup/github/do/upload",
  ...
  @RequestProcessing(
      value = "/prop/backup/hacpai/do/upload",
      method = {HttpMethod.GET})
      ...
      for (TranslateResult i : list) {
        try {
          ...
          final String permalink =
              "/articles/"
                  + DateFormatUtils.format(i.getDate(), "yyyy/MM/dd")
                  + "/"
  ...
  public void importFromMarkdown(final RequestContext context) {
    ...
    try {
      ...
      for (FileItem item : itemList) {
        ...
        item.write(file);
        item.delete();
        ZipFilterUtils.unpackFilteredZip(
            new File(markdownsPath + name), new File(markdownsPath), null);
        file.delete();
      }
