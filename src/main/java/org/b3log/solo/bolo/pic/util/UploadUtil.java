    ...
    switch (type) {
      case "local":
        ...
        try {
          localFilename =
              localDate
                  + file.getName().substring(file.getName().lastIndexOf(".")).replace(" ", "_");
        } catch (Exception e) {
        ...
        LOGGER.log(
            Level.INFO,
            "An image has been uploaded to local [path=" + localNewFile.getAbsolutePath() + "]");
        break;
      case "picuang":
        ...
        try {
          HttpPost httpPost =
              new HttpPost(picuangSite + "/upload/auth?password=" + picuangPassword);
      ...
      case "aliyun":
        ...
        try {
          ...
          aliyunFilename =
              RandomStringUtils.randomAlphanumeric(3) + "_" + file.getName().replace(" ", "_");
        }
        OSS aliyunOSSClient =
            new OSSClientBuilder().build(aliyunEndPoint, aliyunAccessKeyID, aliyunAccessKeySecret);
        PutObjectRequest aliyunPutObjectRequest =
            new PutObjectRequest(aliyunBucketName, aliyunFilename, file);
        try {
      ...
      case "tencent":
        ...
        COSCredentials tencentCosCred =
            new BasicCOSCredentials(tencentCosSecretId, tencentCosSecretKey);
        com.qcloud.cos.region.Region tencentRegion =
            new com.qcloud.cos.region.Region(tencentCosRegion);
        ...
        com.qcloud.cos.model.PutObjectRequest tencentPutObjectRequest =
            new com.qcloud.cos.model.PutObjectRequest(tencentCosBucketName, tencentCosKey, file);
        PutObjectResult tencentPutObjectResult =
            tencentCosClient.putObject(tencentPutObjectRequest);
