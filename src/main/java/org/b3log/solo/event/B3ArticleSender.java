...
/**
 * This listener is responsible for sending article to B3log Rhythm. Sees <a
...
public class B3ArticleSender extends AbstractEventListener<JSONObject> {
  ...
  public static void pushArticleToRhy(final JSONObject data) {
    try {
      ...
      if (StringUtils.isNotBlank(originalArticle.optString(Article.ARTICLE_VIEW_PWD))) {
        LOGGER.log(
            Level.INFO, "Article [title={0}] is a password article, ignored push to Rhy", title);
      ...
      final ArticleQueryService articleQueryService =
          beanManager.getReference(ArticleQueryService.class);
      ...
      if (Option.DefaultPreference.DEFAULT_B3LOG_USERNAME.equals(userName)) {
        LOGGER.log(
            Level.INFO,
            "Article [title={0}] Is using the B3log default account, skipped push to Rhy",
            title);
      ...
      final JSONObject requestJSONObject =
          new JSONObject().put("article", article).put("client", client);
      final HttpResponse response =
          HttpRequest.post("https://rhythm.b3log.org/api/article")
              .bodyText(requestJSONObject.toString())
  ...
  static boolean isLocalServer() {
    return StringUtils.containsIgnoreCase(Latkes.getServePath(), "localhost")
        || Strings.isIPv4(Latkes.getServerHost())
        || (StringUtils.isNotBlank(Latkes.getServerPort()))
            && !"80".equals(Latkes.getServerPort())
            && !"443".equals(Latkes.getServerPort());
  }
