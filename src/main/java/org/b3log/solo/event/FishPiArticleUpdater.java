...
/**
 * This listener is responsible for sending updated article to fishpi community base Rhythm. Sees <a
 * href="https://fishpi.cn"></a> for more details.
 *
...
public class FishPiArticleUpdater extends AbstractEventListener<JSONObject> {
  ...
  public static void putArticleToFishPi(final JSONObject data) {
    try {
      ...
      if (StringUtils.isNotBlank(originalArticle.optString(Article.ARTICLE_VIEW_PWD))) {
        LOGGER.log(
            Level.INFO, "Article [title={0}] is a password article, ignored put to fishpi", title);
        return;
      }
      ...
      if (Option.DefaultPreference.DEFAULT_B3LOG_USERNAME.equals(userName)) {
        LOGGER.log(
            Level.INFO,
            "Article [title={0}] Is using the B3log default account, skipped put to Rhy",
            title);
        return;
      }
      ...
      final JSONObject option =
          optionQueryService.getOptionById("article_" + originalArticle.getString(Keys.OBJECT_ID));
      final String fishPiArticleId = option.optString(Option.OPTION_VALUE);
      if (Objects.isNull(option) || "".equals(fishPiArticleId)) {
        LOGGER.log(
            Level.WARN,
            "Put an article to FishPi failed,Cannot find the reference between this article and fishpi article");
        return;
      }

      final HttpResponse response =
          HttpRequest.put(
                  String.format("https://fishpi.cn/article/%s?apiKey=%s", fishPiArticleId, fishKey))
              .bodyText(article.toString())
              .connectionTimeout(3000)
              .timeout(7000)
              .followRedirects(true)
              .contentTypeJson()
              .header("User-Agent", Solos.BOLO_USER_AGENT)
              .send();

      LOGGER.log(
          Level.INFO,
          "Put an article [title={0}] to FishPi, response [{1}]",
          title,
          response.toString());
  ...
  public void action(final Event<JSONObject> event) {
    ...
    LOGGER.log(
        ...
        event.getType(),
        data,
        FishPiArticleUpdater.class.getName());
