...
/**
 * This listener is responsible for sending article to fishpi community base Rhythm. Sees <a
...
public class FishPiArticleSender extends AbstractEventListener<JSONObject> {
  ...
  public static void pushArticleToFishPi(final JSONObject data) {
    try {
      ...
      if (StringUtils.isNotBlank(originalArticle.optString(Article.ARTICLE_VIEW_PWD))) {
        LOGGER.log(
            Level.INFO, "Article [title={0}] is a password article, ignored push to fishpi", title);
        return;
      }
      ...
      final HttpResponse response =
          HttpRequest.post(String.format("https://fishpi.cn/article?apiKey=%s", fishKey))
      ...
      if (respJson.has("code") && respJson.optInt("code") == 0) {
        ...
        final OptionMgmtService optionMgmtService =
            beanManager.getReference(OptionMgmtService.class);
      ...
      LOGGER.log(
          Level.INFO,
          "Pushed an article [title={0}] to FishPi, response [{1}]",
          title,
          response.toString());
  ...
  public void action(final Event<JSONObject> event) {
    ...
    LOGGER.log(
        ...
        event.getType(),
        data,
        FishPiArticleSender.class.getName());
