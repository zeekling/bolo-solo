...
public class DeleteFollowListener extends AbstractEventListener<JSONObject> {
     ...
     * Logger.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(DeleteFollowListener.class);
    ...
    public void action(final Event<JSONObject> event) {
        ...
        if (null == ret || ret.isEmpty()) {
            LOGGER.warn("delete follow article cache: No follow articles found for follow [name={0}]",
                    followName);
            return;
        }
        LOGGER.info("delete follow article cache: Removed follow articles [followName={0}, articleCount={1}]",
                followName, ret.size());
    }
